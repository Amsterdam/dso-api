...
class BrowsableAPIRenderer(RendererMixin, renderers.BrowsableAPIRenderer):
    ...
    def get_breadcrumbs(self, request):
        ...
        for i, crumb in enumerate(breadcrumbs):
            if (
                i > 0
                and crumb[0] == breadcrumbs[i - 1][0]
                and crumb[1].rstrip("/") == breadcrumbs[i - 1][1].rstrip("/")
            ):
                continue
            filtered_breadcrumbs.append(crumb)

        filtered_breadcrumbs[-1] = (
            self.get_name(self.renderer_context["view"]),
            filtered_breadcrumbs[-1][1],
        )
