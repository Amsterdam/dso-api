...
function setPageLinks(page) {
  ...
  links.forEach(link => {
    link["a"].onclick = e => {
      updateSetting("page", new URL(e.target.href).searchParams.get("page"), "eq", type = "param");
...
function onParamOpSet(event) {
  ...
  if (oaParams.hasOwnProperty(key)) {
    ...
    let oldValueEl = event.target.parentElement.getElementsByClassName("param-val")[0];
...
function createParamEl(key = null, value = null, op = "eq", active = true, insideOf = "query") {
  ...
  activeEl.onchange = (e) => {
    e.target.checked ? e.target.parentElement.classList.remove("disabled") : e.target.parentElement.classList.add("disabled");
  }
...
function setHeaders(headers = null) {
  ...
  if (headers) {
    ...
    if (token) {
      addSetting("Authorization", "Bearer " + token.access_token, "eq", "header", false);
    }
  }
...
function setParams(params = null) {
  ...
  params.forEach(param => {
    let active = (PAGEURL.searchParams.has(param.combinedKey) && PAGEURL.searchParams.get(param.combinedKey) == param.value) || param.key == "";
