...
node {
    ...
    stage('Test') {
        tryStep "test", {
            sh "docker-compose -p dso_api_$(git rev-parse --short HEAD) -f src/.jenkins/test/docker-compose.yml build --pull && " +
               "docker-compose -p dso_api_$(git rev-parse --short HEAD) -f src/.jenkins/test/docker-compose.yml run -u root --rm test"
        }, {
            sh "docker-compose -p dso_api_$(git rev-parse --short HEAD) -f src/.jenkins/test/docker-compose.yml down"
        }
    }   
